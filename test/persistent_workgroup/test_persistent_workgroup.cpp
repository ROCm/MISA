...
std::string parse_base_arg(int argc, char* argv[])
{
    ...
    if(arg != "conv" && arg != "convfp16" && arg != "convint8" && arg != "--version" && arg != "convfp16x8")
    {
...
#define     SCLK_MHZ    2445
#define WARMUP 3
#define REPEAT 8

#ifndef HSACO
#define HSACO "igemm_fwd_nhwc_fp16.hsaco"
#endif
int main(int argc, char **argv){
    ...
    std::string default_hsaco = "igemm_fwd_gtcn2_nchwc_kcyxc_fp16x8.hsaco";
...
#define P_DEVICE_PROP_INT(prop) \
    ...
    printf("num_cu:%d, arch:%d, theo_gflops:%f, n:%d, c:%d, hi:%d, wi:%d, k:%d, y:%d, x:%d, py:%d, px:%d, dy:%d, dx:%d. sy:%d, sx:%d\n",
            num_cu, gcn_arch, theo_gflops, n, c, hi, wi, k, y, x, pad_h, pad_w, dilation_h, dilation_w, stride_h, stride_w);
    ...
    if (need_fwd){
        ...
        if (need_verify) {
            ...
            if(in_layout == "NCHWC"){
                ...
                HIP_CALL(hipMemcpy(aux_out, device_output,
                                   static_cast<size_t>(n) * k * ho * wo * sizeof(float),
                                   hipMemcpyDeviceToHost));

                tensor_transpose_nchw_2_nchwc<float*>(host_output, aux_out, n, k, ho, wo, vector_c);
        ...
        igemm_fwd_t conv_fwd_driver;
        int valid_index = 0;
        result_t fastest_result;
        fastest_result.duration_ms = FLT_MAX;
        int fastest_id = -1;
        for (int i = 0; i < sizeof(igemm_fwd_kernel_list)/sizeof(igemm_fwd_kernel_list[0]); i++) {
            igemm_fwd_kernel_info_t *kinfo = &igemm_fwd_kernel_list[i];
            if(driver_data_type == driverHalf){
