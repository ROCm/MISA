...
v_cmp_le_i32 vcc, 0, v[2]
    v_cmp_gt_i32 s[s_tmp:s_tmp+1], s[s_hi], v[2]
    s_and_b64 vcc, vcc, s[s_tmp:s_tmp+1]
    v_cndmask_b32 v[v_flag], 0, v[v_flag], vcc
    ;   flag: 0<= * <wi
    v_cmp_le_i32 vcc, 0, v[v_tmp+3]
    v_cmp_gt_i32 s[s_tmp:s_tmp+1], s[s_wi], v[v_tmp+3]
    s_and_b64 vcc, vcc, s[s_tmp:s_tmp+1]
    v_cndmask_b32 v[v_flag], 0, v[v_flag], vcc
...
.text
.p2align 8
.amdgpu_hsa_kernel igemm_v4r1_generic_1x1
...
.set s_block_ib,            27
.set s_in_stride,           23
.set s_in_stride_n2,        21
.set s_in_stride_n1,        22
.set s_wei_stride,          29
.set s_wei_stride_k,        28
.set s_out_stride_k0,       30
.set s_out_stride_k1,       31
.set s_out_stride_n1,       32
.set s_out_stride_n2,       33
.set s_kitr,                34
...
; since changing tunable parameter by modify asm source is very hard to achieve
...
igemm_v4r1_generic_1x1:
    ...
    v_lshrrev_b32 v[v_in_ie], 4, v0
    ; wei e_k cluster: {4, 64}, {4, 2}, order:{1,0}
    v_lshrrev_b32 v[v_tmp], 2, v0
    v_lshlrev_b32 v[v_wei_ik], 1, v[v_tmp]
    v_and_b32 v[v_tmp+1], 3, v0
    v_lshlrev_b32 v[v_wei_ie], 2, v[v_tmp+1]
    ...
    s_lshr_b32 s[s_tmp], s[s_n], 3                          ; N0 = N / (N1 * N2);
    s_mul_i32 s[s_tmp+1], s[s_out_stride_k1], s[s_tmp]        ; B = N0 * Ho * Wo;
    s_lshr_b32 s[0], s[s_tmp+1], 4                          ; BBlockWork = B / BPerBlock;
    ...
    s_lshl_b32 s[s_block_ik], s[s_tmp], 7                   ; k_block_data_on_global = KBlockID * KPerBlock;
    s_lshl_b32 s[s_block_ib], s[s_tmp+1], 4                 ; b_block_data_on_global = BBlockID * BPerBlock;

    ; input
    ; 1. e_n1_b_n2:b
    ;   1). transform: b -> n0*ho*wo
    v_add_u32 v[1], s[s_block_ib], v[v_in_ib]
    .v_u32_div_vs 11, 1, s_out_stride_k1, v_tmp, s_tmp
    v_mul_lo_u32 v[v_tmp], s[s_out_stride_k1], v[11]
    v_sub_u32 v[1], v[1], v[v_tmp]
    .v_u32_div_vs 2, 1, s_wo, v_tmp, s_tmp
    v_mul_lo_u32 v[v_tmp], s[s_wo], v[2]
    v_sub_u32 v[v_tmp+3], v[1], v[v_tmp]
    ;   11:n0, 2:ho, v_tmp+3:wo
    ;   2). transform: ho, wo -> hip, wip
    v_mul_lo_u32 v[2], s[s_stride_h], v[2]
    v_mul_lo_u32 v[v_tmp+3], s[s_stride_w], v[v_tmp+3]
    ;   3). transform: hip, wip -> hi, wi
    v_sub_i32 v[2], v[2], s[s_pad_h]
    v_sub_i32 v[v_tmp+3], v[v_tmp+3], s[s_pad_w]
    ...
    ;   offset:
    v_mul_lo_u32 v[v_in_os], s[s_wi], v[2]
    s_mul_i32 s[s_tmp+1], s[s_wi], s[s_hi]
    v_add_u32 v[v_in_os], v[v_in_os], v[v_tmp+3]
    s_lshl_b32 s[s_in_stride], s[s_tmp+1], 4+2              ; Sequence<EPerBlock, 0, 0, 0>
    v_lshlrev_b32 v[11], 3, v[11]
    s_mul_i32 s[s_in_stride_n2], s[s_tmp+1], s[s_c]
    v_mul_lo_u32 v[v_tmp], s[s_in_stride_n2], v[11]
    ...
    ; E = C * 1 * 1
    s_sub_i32 s[s_kitr], s[s_c], t_e_per_block
    s_cmp_gt_i32 s[s_kitr], 0
    s_cbranch_scc0 L_igemm_v4r1_generic_1x1_fma_end
...
L_igemm_v4r1_generic_1x1_fma_body:
    ; do fma accumulate
    .itr_k = 0
    .rept 15
        .if .itr_k == 0
    ...
    s_cmp_gt_i32 s[s_kitr], 0
    s_cbranch_scc0 L_igemm_v4r1_generic_1x1_fma_finishing
    ...
    .v_fma_4x4_s8 v_c+36, v_la+4, v_lb+4
    s_branch L_igemm_v4r1_generic_1x1_fma_body
L_igemm_v4r1_generic_1x1_fma_finishing:
    s_waitcnt lgkmcnt(4)
    .v_fma_4x4_s8 v_c+32, v_la+4, v_lb+0
    .v_fma_4x4_s8 v_c+36, v_la+4, v_lb+4
L_igemm_v4r1_generic_1x1_fma_end:
    s_mov_b32 s[s_tmp+0], 0
...
L_igemm_v4r1_generic_1x1_fma_out:
